 ...
 */
    ...
    void test_point_Methode(){
        ...
        mockMessage.setTimeStamp("2021-05-17 14:52:10:000");
        mockMessage.setMessage("dsaf");
        mockMessage.setWriterName(testUser2.getUsername());

        Chat testChat = new Chat();
        testChat.setChatId(testLobby.getId());
        chatRepository.saveAndFlush(testChat);

        // when
        Long newGameID= gameService.createGame(testLobby);
        Game createdGame = gameService.getGame(newGameID);
        createdGame.setRoundId(5L);

        Round createdRound = roundService.createRound(createdGame);
        roundRepository.saveAndFlush(createdRound);

        gameService.addPoints(createdGame, mockMessage);
